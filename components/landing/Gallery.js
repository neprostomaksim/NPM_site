...
// Фото лежат в public/workshops/gallery. Порядок подобран под masonry (чередование ориентаций).
        ...
        <div className="gallery">
          {PHOTOS.map((src, i) => (
            <Reveal key={src} className="gallery-item" delay={(i % 3) * 0.06}>
              <Image src={`/workshops/gallery/${src}`} alt={`Участники практического воркшопа по ИИ в Минске — фото ${i + 1}`} width={1200} height={1600} sizes="(max-width: 767px) calc(100vw - 40px), 33vw" quality={72} />
            </Reveal>
          ))}
        </div>
      </div>
